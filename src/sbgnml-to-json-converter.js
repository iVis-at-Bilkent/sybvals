...
let options = optionUtilities.getOptions();
...
sbgnmlToJson.portsEnabled = true;
...
sbgnmlToJson.disablePorts = function() {
  sbgnmlToJson.portsEnabled = false;
};

sbgnmlToJson.enablePorts = function() {
  sbgnmlToJson.portsEnabled = true;
};
...
sbgnmlToJson.addCytoscapeJsNode = function (ele, jsonArray, parent, compartments) {
  ...
  if (elementUtilities.canHavePorts(_class)) {
    if (sbgnmlToJson.portsEnabled && ports.length === 2) {
...
sbgnmlToJson.mapPropertiesToObj = function () {
  ...
  if (this.map.extension && this.map.extension.has('mapProperties')) { // render extension was found
    ...
    return { mapProperties: { compoundPadding: self.getCompoundPadding() } };
  }
};

sbgnmlToJson.convert = function (xmlObject, xmlString, urlParams) {
  var self = this;
  ...
  if (urlParams && urlParams.compoundPadding) {
    self.setCompoundPadding(Number(urlParams.compoundPadding));
  }
    ...
    self.setCompoundPadding(newPadding);


  } else {
    self.setCompoundPadding(Number(self.mapPropertiesToObj().mapProperties.compoundPadding));
  }
...
sbgnmlToJson.setCompoundPadding = function (newPaddingValue) {
  options.compoundPadding = newPaddingValue;
  optionUtilities.extendOptions(options);
};

sbgnmlToJson.getCompoundPadding = function () {
